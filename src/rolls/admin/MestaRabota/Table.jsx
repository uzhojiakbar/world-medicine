...
const Table = ({ data = [], loading = true, setLoading = () => { } }) => {
  ...
  const currentData = curData.slice(
  ...
  const handleSave = async () => {
    ...
    setCurData(curData?.map(v => {
      return v.id == editedRow.id ? editedRow : v
    }))
    setEditId(null);
  ...
  const handleDelete = (id) => {
    setCurData(curData.filter(v => v.id !== id))
  }
              ...
              return <Details key={i}>
                ...
                {editId == row.id ?
                  <Input
                    type="text"
                    name="Район"
                    value={editedRow.subTitle}
                    onChange={(value) =>
                      handleInputChange("subTitle", value)
                    }
                  ...
                  {editId === row.id ? (
                    <>
                      <button
                        ...
                        onClick={handleSave}
                      >
                        <i class="fa-solid fa-floppy-disk colorBlue" style={{ color: "#216BF4", fontSize: "20px" }}></i>
                      </button>
                      <button
                        style={{ background: "transparent" }}
                      ...
                      >
                        <i class="fa-solid fa-close colorRed" style={{ color: "red", fontSize: "20px" }}></i>
                      </button>
                    </>
                  ) : (
                    <>
