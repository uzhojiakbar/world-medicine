...
const ModalDoctor = ({user, isOpen, onClose}) => {
    ...
    const {data: regions, isLoading: isRegionLoading} = useGetRegions();const {
        data: districts,
    ...
    const {data: workplaces, isLoading: isWorkplacesLoading} = useGetWorkplaces({
        regionId: district?.regionId || null, districtId: district?.districtId || null,
    });
    ...
    return ((!user || isDistrictLoading || isRegionLoading || isDistrictsLoading || isWorkplacesLoading) ?
            ...
            >
                ...
                <ModalBodyHeader>
                    ...
                    <ModalBodySection>
                        ...
                        <ModalInnerSection>
                            <EditableInput initialValue={user?.dateOfCreation?.split("T")[0]} isInput={0}
                                           inputType="text"/>
                        </ModalInnerSection>
                    </ModalBodySection>
                    <ModalBodySection>
                ...
                <AnimatePresence>
                    ...
                    {inDetail ? doctorContract ? <motion.div
                            initial="hidden"
                            animate="visible"
                            exit="hidden"
                        ...
                        >
                            ...
                            <ModalBodyHeader gridC={2}>
                                {doctorContract?.contractedMedicineWithQuantity?.length > 0 ? editContract ? <InfoWrapper
                                    bg={'#F7F8FC'}
                                >
                                    ...
                                    {doctorContract?.contractedMedicineWithQuantity?.map((v) => {
                                        ...
                                        return <Item bg={"white"} className="itemInner">
                                            ...
                                            <div
                                                style={{
                                                    display: "flex", userSelect: "none", gap: "10px",
                                                }}
                                            >
                                                {v?.quote}
                                                <svg
                                ...
                                {doctorContract?.outOfContractMedicineAmount?.length > 0 ? <InfoWrapper>
                                    <TitleSmall size={"18px"}>
