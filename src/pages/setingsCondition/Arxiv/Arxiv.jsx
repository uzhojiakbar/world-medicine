...
import { useGetAllContract2 } from "../../../utils/server/server2.js";
...
const Arxiv = () => {
  ...
  const { data, isLoading: loading } = useGetAllContract2(0);

  const filteredData = data?.content?.filter((item) => {
    if (!item.user) return false;
    const searchWords = nameSurname?.toLowerCase().trim().split(/\s+/) || [];
    ...
    return (
      (!selectedViloyat ||
        item.user?.regionDistrictDTO?.regionId === selectedViloyat) &&
      (!selectedTuman ||
        item.user?.regionDistrictDTO?.districtId === selectedTuman) &&
      (!selectedSpecialization ||
        item.user?.fieldName === selectedSpecialization) &&
      (!nameSurname || searchWords.every((word) => fullname.includes(word)))
    );
  });
  ...
  return (
    <Wrapper>
      <Filter
        ...
        setSelectedViloyat={setSelectedViloyat}
        setSelectedTuman={setSelectedTuman}
      ...
      <MainTable title="rahmadjon" data={filteredData} loading={loading} />
    </Wrapper>
  );
};
