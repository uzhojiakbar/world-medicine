import { ModalBody, ModalContainer } from "../../../root/Modal";
...
const ModalSuccessful = ({ isOpen, setOpen = () => {} }) => {
  ...
  const rePassHandle = () => {
    if (pass?.newPass === pass?.curPass && pass?.newPass?.length > 5) {
      ...
      message.error(
        translate(
          "parollar bir xil bolsin va minimal 6 xonadan iborat bolishi kerak"
        )
      );
  };
    ...
    >
      <InfoContainer>
        ...
        <div size={"16px"} className="dfn">
          {translate(
            "После вы получите временный пароль, который нужно будет сменить после входа в аккаунт"
          )}
          .
        </div>
        ...
        <div className="inputWrapp">
          <InputWrapper pad={"none"} bgcolor="white">
            <Input
              type={pass?.newPassType ? "password" : "text"}
              value={pass?.newPass?.length < 0 ? "" : pass?.newPass}
            ...
            <ForSee
              onClick={() =>
                setPass({ ...pass, newPassType: !pass?.newPassType })
              }
            />
          </InputWrapper>
          <InputWrapper pad={"none"} bgcolor="white">
            <Input
              type={pass?.curPassType ? "password" : "text"}
              value={pass?.curPass?.length < 0 ? "" : pass.curPass}
            ...
            <ForSee
              onClick={() =>
                setInputType(
                  setPass({ ...pass, curPassType: !pass?.curPassType })
                )
              }
            />
          </InputWrapper>
      ...
      <ModalBody>
        ...
        <ResetPassword
          onClick={() => rePassHandle()}
          bgcolor={
            pass?.curPass <= 0 || pass?.curPass != pass?.newPass
              ? "#f77682"
              : "#fb3748"
          }
        >
