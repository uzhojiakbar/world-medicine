...
const PurposeMedAgent = () => {
  ...
  const [preparats, setPreparats] = useState([
  ...
  const handleSubmit = (e) => {
    ...
    //   },
    //   body: JSON.stringify({ name }), // yuborilayotgan ma'lumot
    // })
    //   .then((response) => response.json())
    //   .then((data) => {
    //     console.log('Data submitted:', data);
    //     // Ma'lumotni yuborganidan keyin nimadir qilish
    //   })
  ...
  return (
    <Container>
      <Wrapper>
        <Title>{translate("Цель мед. представителю")}</Title>
        ...
        <InfoWrapper>
          <InfoContainer>
            ...
            <Wrap>
              <Title size="24px">{translate("Выбрать район")}</Title>
              <PrimarySelect
                def={translate("Выбрать район")}
                options={Tumanlar["Наманган"]}
              />
            </Wrap>

            <Wrap>
              <Title size="24px">{translate("Выбрать договор")}</Title>
              <PrimarySelect
                def={translate("Выбрать договор")}
                options={[{ value: "Договор №1" },
                { value: "Договор №2" },
                { value: "Договор №3" },
                ]}
              />
          ...
          <InfoContainer>
            <Wrap>
              <Title size="24px">{translate("Охват врачей")}</Title>
              <PrimarySelect
            ...
            <Wrap>
              {doctorsType.map((v, i) => {
                return (
                  <ItemContainer key={i}>
                    <Child onClick={() => setDoctorsType(doctorsType.filter((v, ind) => ind !== i))} >
                      <svg
                    ...
                    <Child>
                      <span>{v.number}</span>
                      <EditIconCon onClick={() => handleEdit(i)}>
                        <svg
                          width="16"
                          height="16"
                          viewBox="0 0 24 24"
            ...
            <InfoContainer>
              <Wrap>
                <Title size="24px">{translate("Заключение договоров")}</Title>
                <PrimarySelect
                  def={translate("Выберите препарат")}
                  options={[
                    // { value: "Алтикам" },
                    { value: "Алтикам" },
                    { value: "Ампилин" },
              ...
              <Wrap>
                {preparats.map((v, i) => {
                  return (
                    <ItemContainer key={i}>
                      <Child onClick={() => {
                        setPreparats(preparats?.filter((v, ind) => ind !== i))
                      }
                      }>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
...
export default PurposeMedAgent;
